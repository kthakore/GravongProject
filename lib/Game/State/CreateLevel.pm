...
sub _drawing_mouse_handler
{
	...
	{
		$self->planets->[ $self->{current_planet} ]->{ size } += 1; 
	}
	if( $self->current_state eq 'placing' && $event->type == SDL_MOUSEBUTTONUP )
	{
...
sub _draw_planets
{
	...
	foreach my $planet (@planets)
	{
		...
		$app->draw_circle_filled( [$planet->{x}, $planet->{y}], $planet->{size}, [255,0,0,255] ); 
